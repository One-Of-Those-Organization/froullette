...
public:
  ...
  void render() override {
    ...
    if (str != "" || !text) {
      DrawTextPro(*font, str.c_str(), Vector2(rec.x + padding, rec.y + padding),
                  Vector2(0, 0), 0.0f, str_size, _spacing, fgcolor);
    } else {
      DrawTexturePro(*text, Rectangle(0, 0, text->width, text->height), rec,
                     Vector2(0, 0), 0.0f, fgcolor);
    }
  }
  ...
  int _get_width() {
    if (font) {
      Vector2 size = MeasureTextEx(*this->font, this->str.c_str(),
