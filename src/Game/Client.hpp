    ...
    do {                                                                       \
        if (g_ws_handle) {                                                     \
            if ((op) == WEBSOCKET_OP_TEXT) {                                   \
                emscripten_websocket_send_utf8_text(g_ws_handle,               \
                                                    (const char *)(data));     \
            } else {                                                           \
                emscripten_websocket_send_binary(                              \
                    g_ws_handle, (const char *)(data), (int)(len));            \
            }                                                                  \
        }                                                                      \
    } while (0)
        ...
        if (_len_ > 0) {                                                       \
            if (g_ws_handle) {                                                 \
                if ((op) == WEBSOCKET_OP_TEXT) {                               \
                    ...
                    emscripten_websocket_send_binary(g_ws_handle, _buf_,       \
                                                     _len_);                   \
                }                                                              \
            }                                                                  \
        }                                                                      \
...
class Client {
  public:
    ...
    bool connect(void *data) {
        ...
        this->c = mg_ws_connect(&this->mgr, this->_buffer, this->callback, data,
                                NULL);
        ...
        emscripten_websocket_set_onopen_callback(g_ws_handle, this,
                                                 Client::on_open_ems);
        emscripten_websocket_set_onmessage_callback(g_ws_handle, this,
                                                    Client::on_message_ems);
        emscripten_websocket_set_onclose_callback(g_ws_handle, this,
                                                  Client::on_close_ems);
        emscripten_websocket_set_onerror_callback(g_ws_handle, this,
                                                  Client::on_error_ems);
  ...
  private:
            ...
            if (g_ws_handle) {
                if (msg.opcode == WEBSOCKET_OP_TEXT) {
                    emscripten_websocket_send_utf8_text(
                        g_ws_handle, (const char *)msg.data.data());
                } else {
                    emscripten_websocket_send_binary(g_ws_handle,
                                                     (void *)msg.data.data(),
                                                     (int)msg.data.size());
                }
            }
...
#ifdef __EMSCRIPTEN__
    static EM_BOOL on_open_ems(int eventType,
    ...
    static EM_BOOL on_message_ems(int eventType,
    ...
    static EM_BOOL on_close_ems(int eventType,
    ...
    static EM_BOOL on_error_ems(int eventType,
