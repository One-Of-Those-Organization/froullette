...
static void client_handler(mg_connection *c, int ev, void *ev_data) {
    ...
    if (!gd) {
        TraceLog(LOG_INFO,
                 "NET: Failed to get the gamedata on the network thread");
        return;
    }
    ...
    case MG_EV_WS_MSG: {
        ...
        while (off < len) {
...
#endif
                gd->room =
                    pd.data.Room_obj; // this allocate mem dont forget to free
            ...
            case GAME_TURN_UPDATE: {
                ...
                gd->room->turn =
                    (PlayerState)pd.data
                        .Boolean; // use smaller one bro it doesnt need to use
                                  // int because thats already 4 byte.
                TraceLog(LOG_INFO, "NET: Turn Update received: %d",
                         pd.data.Int);
            } break;
...
static void initInGame(ArsEng *engine, int kh_id, int *z) {
    ...
    auto p2 = new Object();
    p2->rec = Rectangle{0, 0, player2_text->width / 8.0f,
                        player2_text->height / 8.0f};
...
static void initMenu(ArsEng *engine, int kh_id, int *z) {
    ...
    Text *title1 =
        cText(engine, state, "Fate", title_size, title_color, {0, 0});
...
static void initPlayMenu(ArsEng *engine, int kh_id, int *z) {
    ...
    if (!kh)
        TraceLog(LOG_INFO,
                 "Failed to register keybinding to the playmenu state");
    else {
    ...
    Button *btncreate = cButton(engine, "Create room", text_size, padding,
                                state, {0, 0}, [engine, gd]() {
                                    if (!gd->client->c) {
...
static void initSettings(ArsEng *engine, int kh_id, int *z) {
    ...
    if (!kh)
        TraceLog(LOG_INFO,
                 "Failed to register keybinding to the settings state");
    else {
        kh->add_new(KEY_Q, state, [engine]() { engine->revert_state(); });
    }
    ...
    Button *btnhd =
        cButton(engine, "720p", text_size, padding, state, {0, 0},
...
static void initRoomMenu(ArsEng *engine, int kh_id, int *z) {
    ...
    Button *btn1 = cButton(engine, "", 0, padding, state, {0, 0}, [engine]() {
        ...
        if (gd->room) {
            delete gd->room;
            gd->room =
                nullptr; // NOTE: IDK if this is the best approach but yeah...
        }
        engine->revert_state();
    });
    btn1->text = exit_icon;
...
static void initALLObject(ArsEng *engine, int kh_id, int *z) {
    ...
    Text *t =
        cText(engine, state, std::string(), text_size, text_color, {0, 0});
